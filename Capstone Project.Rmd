...
fx_list <- paste(seq(from = 812, to = 814, by = 0.01), collapse = "|")
...
diagnosis_df %>% 
  summarize.(diag_codes = paste(diag_code, collapse = " "),
             ...
             by = c(id)) %>% 
  mutate.(fx_frac = str_detect(diag_codes, pattern = fx_list)) %>% 
  select.(id, fx_frac, diag_codes, patient_diagnosis) 
...
patient_df <- pto_df %>%
  ...
  mutate.(dob = mdy(dob),
          ...
          fall_height = fall_height_func(fall_height)
  )
