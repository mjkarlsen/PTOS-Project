...
complications_df <- complications_full_df %>% 
  summarize.(complication_codes = paste(complication_code, collapse = " "),
             patient_complications = paste(complication_desc, collapse = ", "),
             by = c(id)) %>% 
  mutate.(comp_syndrome = str_detect(complication_codes, pattern = '32')) %>% 
  select.(id, comp_syndrome, complication_codes, patient_complications) 
...
procedures_df <- pto_df %>% 
  ...
  mutate.(date = mdy(da), 
          ...
          # date_time = ymd_hm(ifelse(!is.na(time),paste(date, time),paste(date, "12:00"))),
