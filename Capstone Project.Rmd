...
# rename_function <- function(ptos_data) {
#   new_df <- ptos_data
#   
for(i in colnames(ptos_test)){

  old_col_name <- names(ptos_test[,..i])
  new_col_name <- case.(old_col_name == 'state_pa', 'STATE',
                        default = old_col_name)

  setnames(ptos_test[,..i], old_col_name, new_col_name)
}
# 
#   return(new_df)
# }
#   
# new_ptos <- rename_function(ptos_test)
# new_ptos
...
rename_function <- function(.data) {
  
  for(i in 1:length(colnames(.data))){
    new_data <- .data
    
    old_names <- as.character(names(.data))
    new_names <- case.(old_names == 'alias_inst_num', 'YES_FINALLY',
                       old_names == 'trauma_num', 'DRAMA', 
                       old_names == 'e_code', 'Super_CODE',
                       default = old_names)
...
rename_function(ptos_test)


source("01 - Classification Functions.R")

ptos_test_new <- raw_data %>%  
  select.(-any_of.(drop_column_list)) 


ptos_test_new %>% ptos_rename_columns()



ptos_test_new %>% head(10) %>% view()
