...
fx_list <- patient_periods %>% 
  ...
  select.(id, fltr_fasciotomy, fltr_procedure, sex, race, age_in_yrs, place_of_injury, cause_of_injury_e_code) %>% 
  distinct.()

fx_proc <- trans_full_df %>% 
  mutate.(data_source = as_factor(data_source), 
          data_source = fct_relevel(data_source, c('arrival', 'diagnosis', 'procedure', 'complications', 'discharge' ))) %>% 
...
fx_proc <- fx_proc %>% 
  ...
  filter.(data_source %in% c('procedure', 'diagnosis', 'discharge', 'complications', 'arrival'))  %>% 
...
model_df <- fx_proc %>% 
  ...
  # select.(fltr_fasciotomy, sex, race, age_in_yrs ) %>% 
  # filter.(date != 'character(0)') %>% 
  mutate.(fltr_fasciotomy = ifelse.(fltr_fasciotomy == T, 1, 0)) %>%
  mutate.( #fltr_fasciotomy = as_factor(fltr_fasciotomy),
          date = lubridate::mdy(date), 
          dow = lubridate::wday(date, label = TRUE)) %>%
  select.(-date, -time, -index, -cause_of_injury_e_code, -code_cd) %>% 
  as_tibble() 
# %>% 
...
lr_rec <-  recipe(fltr_fasciotomy ~ ., data = train_df) %>% 
...
lr_pred <- predict(lr_fit, test_df) %>% 
  bind_cols(test_df)


lr_pred %>% 
  roc_curve(truth = fltr_fasciotomy, .pred_class ) %>% 
  autoplot()
