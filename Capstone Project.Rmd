...
trans_full_test <- ptos::create_full_trans(ptos_df)
trans_flat_test <- ptos::create_flat_trans(trans_full_test)
...
create_flat_trans(trans_full_df)

```


```{r}
test_full_run <- function(.data) {
...
test_full_run(raw_data)

ptos_df %>% select.(id, starts_with.("proc_"))
...
# Use PTOS Package to Cleanup Raw Data

```{r ptos_pkg_results, message=TRUE}
# ptos_df <- create_ptos_data(raw_data)
# 
# create_full_trans(ptos_df)
# create_flat_trans(trans_full_df)
...
rm('trans_full_df')
rm('trans_full_test')
rm('create_flat_trans')
rm('create_full_trans')
...
full_trans_test %>% 
  ...
  drop_na.() %>% 
  mutate.(fltr_diagnosis = str_detect(code_cd_diagnosis, pattern = forearm_fx_list), 
          fltr_procedure = str_detect(code_cd_procedure, pattern = forearm_proc_list), 
...
create_diag_df <- function(.data) {
  ...
  .data <- .data %>%
    ...
    rename.(date = ref_ar_d_a,
            time = ref_ar_t_a,
            code_cd = diag_code,
            code_desc = short_desc) %>% 
    select.(id, date, time, code_cd, code_desc)

  return(.data)
}


diag_test <- create_diag_df(ptos_df)

diag_test %>% head(10) %>% view()
...
pto_df <- raw_data %>% 
  ...
  mutate.(dob = dob_cleanup(age, dob), # ifelse(is.na(dob), as.character(mdy(injury_dt)-years(age)), dob),
          age = age_cleanup(age, dob, injury_dt), #ifelse(is.na(age) & !is.na(dob), as.double(round(difftime(mdy(injury_dt), mdy(dob), unit="weeks")/52.25,0)),age),
...
pto_df <- pto_df %>% 
  mutate.(dob = mdy(dob),
          ...
          ref_facility_trans = transportation_func(ref_facility_trans), 
...
```{r referring_facility_diagnosis}

#NORMALIZE ALL DIAGNOSIS RESULTS BY PATIENT ID
ref_diag_full_df <- pto_df %>% 
  select.(id, starts_with.('ref_')) %>% 
  pivot_longer.(cols = -c(id, 
                          ref_facility_adm_dt, 
                          ref_facility_adm_time, 
                          ref_facility_disc_dt, 
                          ref_facility_disc_time, 
                          ref_facility_desc),
                values_to = "proc_code", 
                values_drop_na = TRUE
                ) %>% 
  filter.(str_detect(name, pattern = 'ref_intd')) %>% 
  mutate.(proc_code = as.double(proc_code)) %>% 
  left_join.(icd_data, by = "proc_code") %>% 
  rename.(date = ref_facility_adm_dt, 
          time = ref_facility_adm_time, 
          location = ref_facility_desc,
          code = proc_code, 
          code_desc_long = procedure_desc, 
          code_desc_short = short_desc) %>% 
  select(id, date, time, location, code, code_desc_long, code_desc_short)
...
test_diag <- create_diag_df(ptos_df)
full_trans_test <- create_full_trans(ptos_df)
...
test_create_diag_df <- function(.data) {
  .data <- .data %>%
    select.(id,
            ref_ar_d_a,
            ref_ar_t_a,
