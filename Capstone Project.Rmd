...
```{r example_for_fairbanks}


pto_df %>% str()
pto_df %>% select.(starts_with.("cs")) 
...
```{r cleansing_func, eval=FALSE, include=FALSE}


rotate_clean_df <- function(unq_id, df, which_columns){


proc_df <- df %>% 
  select.(unq_id, starts_with.('proc_')) %>%  
  pivot_longer.( cols = -unq_id ,
                 values_to = "values", 
                 values_drop_na = TRUE) %>% 
