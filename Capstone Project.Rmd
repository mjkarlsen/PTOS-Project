...
patient_col_list <- list('sex', 'race', 'age_grp', 'peds_flag', 'payor_class_primary', 'injury_month',  'primary_injury_type', 'place_of_injury')
...
#TODO - Added this to the traumaR package and need to remove from here
patient_df <- patient_df %>% mutate.(age_grp = as.character(floor(age_in_yrs/10)), 
                                     peds_flag = ifelse.(age_in_yrs <= 16, T, F), 
                                     arrival_month = ifelse.(!is.na(arrival_dt), NULL, lubridate::month(arrival_dt, label = TRUE)))
