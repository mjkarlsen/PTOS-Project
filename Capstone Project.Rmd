...
pto_df <- raw_data %>% 
  ...
  mutate.(dob = ifelse(is.na(dob), as.character(mdy(injury_dt)-years(age)), dob), 
    id = paste0(str_remove_all(dob, "/"), zip_code, trauma_number)) %>% 
  relocate.(id, .before = alias_inst_num)

pto_df %>% 
  filter.(age %in% NA) %>% 
  select.(dob, age, injury_dt) %>% 
  mutate.(new_age = ifelse(is.na(age) & !is.na(dob), as.integer(interval(mdy(dob), mdy(injury_dt))), F ))


as.character(interval(mdy(injury_dt), mdy(dob))), age
