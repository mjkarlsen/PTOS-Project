...
mods<- list("rf_model", "xgb_model",  "glm_model",  "nnet_model", "ridge_model" )
file_path <- 'E:/Northwestern/12 - Capstone/PTOS_Data/'

for(d in mods) {
    saveRDS(d, file=paste0(file_path,d, ".rds"))
}
...
for(d in mods) {
  if (exists(d)) next(d) 
  else readRDS(file = paste0(file_path,d, ".rds"))
}
...
my_df <- tibble( cause_of_injury_e_code = "Oth MVA Traffic, Highway Collision - Driver of MV, Non MC", 
                 age_grp = '2',
                 code_desc = "Op red-int fix rad/ulna",
                 subchapter_desc = "Reduction of fracture and dislocation",
                 place_of_injury = "Home",
                 sex = "MALE", 
                 race = "White",
                 admission_gcs_eye  = "None", 
                 admission_gcs_motor = "None", 
                 admission_gcs_verbal = "None", 
                 )

my_df

predict(rf_model, my_df, type = "prob")
