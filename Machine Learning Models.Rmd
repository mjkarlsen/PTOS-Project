...
mmdata_results <- mmdata(scores, labels, modnames = model_names)

mmcurves <- evalmod(mmdata_results, raw_curves = TRUE)

autoplot(mmcurves)


aucs <- evalmod(scores = scores, labels = labels, modnames = model_names, mode = 'aucroc')

# Convert to data.frame
aucs.df <- as.data.frame(aucs) %>% 
  rename(test_auc = aucs)

aucs.df %>% 
  select(modnames, aucs) %>% 
  arrange(-aucs) %>% 
  gt() %>% 
  tab_header(title = md("Area Under the Curve (AUC) Scores"),
             subtitle = md("Train vs. Test")) %>% 
  fmt_percent(vars(aucs), 
              decimals = 2)
...
autoplot(curves_test)
autoplot(curves_train)
