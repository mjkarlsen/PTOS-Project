...
patient_df %>% 
  ...
  summary_rows(columns = vars( col_yes, col_no, total_prct),
               ...
               decimals = 1) %>% 
  summary_rows(columns = vars( row_yes, row_no),
               fns = list(Mean = ~mean(.),
                          SD = ~sd(.)),
               formatter = fmt_percent,
               decimals = 1) %>% 
...
prct_of_row_total <- function(x, grand_total) {
  # scales::label_percent(accuracy = 0.01) 
  col_value <- (sum(x)/grand_total)
...
patient_df %>% 
  filter.(peds_adult_flag == 'Peds', 
          fltr_procedure == T) %>% 
  summarize.(cnt = n.(), by = c(peds_adult_flag, fltr_fasciotomy, forearm_fx_desc)) %>% 
  pivot_wider.(names_from = fltr_fasciotomy, values_from = cnt)


patient_df %>% 
  filter.(peds_adult_flag == 'Peds',) %>% 
  select.(peds_adult_flag, fltr_fasciotomy, fltr_procedure) %>% 
  mutate_across.(is.logical, as.integer) %>% 
  t_stat(formula = fltr_fasciotomy ~ fltr_procedure, order = c(1, 0))
...
fx_association_df %>% 
  filter.(code_desc %like% 'Venous cath NEC') %>% 
